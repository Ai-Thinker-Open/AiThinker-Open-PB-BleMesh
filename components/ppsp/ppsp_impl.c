...
#define PPSP_IMPL_CFGS_PROG_VERS_REVI   (5)
...
ppsp_impl_prc_msgs_vers(uint8* msgs)
{
    ...
    uint8   padd;
    ppsp_impl_get_pkcs_7pad(16, 5, padd);
    osal_memset(text, padd, sizeof(text));
    // LOG("%s, pkcs#7=%02x\n", __func__, padd);
    ...
    uint8* xfer = ppsp_impl_new_msgs_resp(msgn, PPSP_IMPL_CFGS_OPCO_VERS_RESP, cipr, sizeof(cipr));
    ppsp_impl_set_msgs_encr(xfer, encr);
    if ( 0 != xfer ) ppsp_impl_psh_msgs_xfer(xfer, 20);
...
ppsp_impl_prc_msgs_upda(uint8* msgs)
{
    ...
    uint32  offs = 0;
    osal_memcpy(&__ppsp_impl_upda_type, text+offs, sizeof(__ppsp_impl_upda_type)); // fw type
    offs += sizeof(__ppsp_impl_upda_type);
    osal_memcpy(&__ppsp_impl_upda_vers, text+offs, sizeof(__ppsp_impl_upda_vers)); // fw version
    offs += sizeof(__ppsp_impl_upda_vers);
    osal_memcpy(&__ppsp_impl_upda_fwsz, text+offs, sizeof(__ppsp_impl_upda_fwsz)); // fw size in byte
    offs += sizeof(__ppsp_impl_upda_fwsz);
    osal_memcpy(&__ppsp_impl_upda_crcs, text+offs, sizeof(__ppsp_impl_upda_crcs)); // crc16
    offs += sizeof(__ppsp_impl_upda_crcs);
    osal_memcpy(&__ppsp_impl_upda_flag, text+offs, sizeof(__ppsp_impl_upda_flag)); // flag: 0:full update, 1:incr update
    printf("%s, type:%x,vers:%x,size:%x,crcs:%x,flag:%x\n",
    ...
    printf("%s, total sctr:%x@size:%x erased!! \n", __func__, sctr, __ppsp_impl_upda_fwsz);
    ...
    uint8   padd;
    ppsp_impl_get_pkcs_7pad(16, 6, padd);
    osal_memset(text, padd, sizeof(text));
    // LOG("%s, pkcs#7=%x\n", __func__, padd);

    text[0] = 0x01; // 1: upda allow, 0: upda deny
    osal_memcpy(text+1, &__ppsp_impl_upda_dnsz, sizeof(__ppsp_impl_upda_dnsz));
    // text[1] = 0x00; // last upda posi
    // text[2] = 0x00; // last upda posi
    // text[3] = 0x00; // last upda posi
    // text[4] = 0x00; // last upda posi
    text[5] = 0x0F; // maxi frame in pack(0x00~0x0f:1~16)
    ...
    uint8* xfer = ppsp_impl_new_msgs_resp(msgn, PPSP_IMPL_CFGS_OPCO_UPDA_RESP, cipr, sizeof(cipr));
    ppsp_impl_set_msgs_encr(xfer, encr);
    if ( 0 != xfer ) ppsp_impl_psh_msgs_xfer(xfer, 20);
...
ppsp_impl_prc_msgs_pack(uint8* msgs)
{
    ...
    LOG("%s, crci:%04x, dnsz:%08x", __func__, __ppsp_impl_upda_crcd, __ppsp_impl_upda_dnsz);
    ...
    text[0] = ((alln&0x0F)<<4)|((seqn&0x0F)<<0); //
    osal_memcpy(text+1, &__ppsp_impl_upda_dnsz, sizeof(__ppsp_impl_upda_dnsz));
    // text[1] = 0x00; // last upda posi
    ...
    uint8* xfer = ppsp_impl_new_msgs_resp(msgn, PPSP_IMPL_CFGS_OPCO_PACK_RESP, cipr, sizeof(cipr));
    ppsp_impl_set_msgs_encr(xfer, 1);
    if ( 0 != xfer ) ppsp_impl_psh_msgs_xfer(xfer, 20);
...
ppsp_impl_prc_msgs_comp(uint8* msgs)
{
    ...
    if ( (0x01 == encr && 0x10 != frsz) || (0x00 == encr && 0x01 != frsz) ) {
        LOG("%s, !! INVALID MESG CONTENT !!", __func__);
        goto RSLT_FAIL_MESG;
    }
    ...
    text[0] = ((__ppsp_impl_upda_crcd == __ppsp_impl_upda_crcs) ? 0x01 : 0x00); //1:succ, 0:fail
    LOG("%s, complete of bins: crcs comp:%s \n", __func__, text[0]?"SUCC": "FAIL");
    ...
    uint8* xfer = ppsp_impl_new_msgs_resp(msgn, PPSP_IMPL_CFGS_OPCO_COMP_RESP, cipr, sizeof(cipr));
    ppsp_impl_set_msgs_encr(xfer, encr);
    if ( 0 != xfer ) ppsp_impl_psh_msgs_xfer(xfer, 20);
...
ppsp_impl_serv_rcvd_hdlr(uint8 para, uint8 coun)
{
    LOG("[PANDA][ENT] %s para:%d, coun:%d", __func__, para, coun);
