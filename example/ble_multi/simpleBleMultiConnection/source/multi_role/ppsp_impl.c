...
__attribute__ ((aligned (4))) uint8   __ppsp_impl_upda_buff[PPSP_DATA_LEN];
...
ppsp_impl_era_prog_data(uint32 addr)
{
	...
	AT_LOG("%s,flash erase 0x%X\n",__func__,PPSP_IMPL_CFGS_PROG_ADDR_BASE + (addr&0xFFF000));	
    flash_sector_erase(PPSP_IMPL_CFGS_PROG_ADDR_BASE + (addr&0xFFF000));
//	flash_block64_erase(PPSP_IMPL_CFGS_PROG_ADDR_BASE + (addr&0xFF0000));
	LOG("flash_sector_erase done \n");
...
//    if ( 0 == ProgramWord(PPSP_IMPL_CFGS_PROG_ADDR_BASE + addr, (uint8*)&valu, sizeof(valu)) ) {
    if ( 0 == ProgramWord(PPSP_IMPL_CFGS_PROG_ADDR_BASE + addr, buf, sizeof(valu)) ) {
...
ppsp_impl_prc_msgs_upda(uint8* msgs)
{
	...
	LOG("sctr %d\n",sctr);
//	uint32 irqV = 0;
//	for ( uint32 itr0 = 0; itr0 < sctr;  ) {
//        // PPSP_IMPL_CFGS_PROG_ADDR_BGNS + sctr * 0x1000
//		irqV = *((volatile uint32_t *)(0xe000e200));
//        LOG("sctr %d,irqV 0x%X\n",itr0,irqV);
//		if( irqV & 0x04 )
//		{
//
//		}
//		else
//		{
//			*((volatile uint32_t *)(0xe000e100)) &= (~0x04);
//			ppsp_impl_era_prog_data(PPSP_IMPL_CFGS_PROG_ADDR_BGNS + (itr0<<12));
////			ppsp_impl_era_prog_data(PPSP_IMPL_CFGS_PROG_ADDR_BGNS + (itr0<<16));
//			itr0 += 1;
//			*((volatile uint32_t *)(0xe000e100)) |= 0x04;
//		}
//    }
...
ppsp_impl_prc_msgs_pack(uint8* msgs)
{
    ...
    uint8   alln;
    uint8   seqn;
    uint16   frsz;   // payload size in byte
    uint8*  plds;   // payloads
    ...
    while ( offs < frsz ) {
		...
		LOG("offs %d,frsz %d,size %d,__ppsp_impl_upda_offs %d\n",offs,frsz,size,__ppsp_impl_upda_offs);
        ...
        if ( sizeof(__ppsp_impl_upda_buff) == __ppsp_impl_upda_offs ) {
			...
			LOG("%s --> flsh pack dwrd %d,__ppsp_impl_upda_offs %d\n",__func__,dwrd,__ppsp_impl_upda_offs);
			LOG("__ppsp_impl_upda_buff %p\n",__ppsp_impl_upda_buff);
            for ( int itr0 = 0; itr0 < dwrd; itr0 += 1 ) {
                uint32 valw=0, valr;
                // push for write
                valw = (( *((uint8*)__ppsp_impl_upda_buff + (itr0<<2)) & 0x000000FF ) | \
                		 (((*((uint8*)__ppsp_impl_upda_buff+1 + (itr0<<2))) << 8 )   & 0x0000ff00) | \
                		 (((*((uint8*)__ppsp_impl_upda_buff+2 + (itr0<<2))) << 16 )   & 0x00ff0000) | \
                		 (((*((uint8*)__ppsp_impl_upda_buff+3 + (itr0<<2))) << 24 )   & 0xff000000));
//				LOG("itr0 %d,valw 0x%X \n",itr0,valw );
...
//	valu = ReadFlash(PPSP_IMPL_CFGS_PROG_ADDR_BASE+PPSP_IMPL_CFGS_PROG_ADDR_BGNS+itr0);
//	LOG("%x ",valu);
	...
	LOG("__ppsp_impl_upda_crcd 0x%X,__ppsp_impl_upda_crcs 0x%X\n",__ppsp_impl_upda_crcd,__ppsp_impl_upda_crcs);
...
ppsp_impl_ini(void)
{
	...
	flash_block64_erase(0x11050000);
	flash_block64_erase(0x11060000);
	flash_block64_erase(0x11070000);
