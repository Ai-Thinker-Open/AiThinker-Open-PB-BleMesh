...
static uint16 ppsp_connHandle=0;
/*********************************************************************
                                 ...
                                 uint8 *pValue, uint16 len, uint16 offset )
{
  ...
  LOG("[PANDA][ENT] %s: len:%d, offs:%d ppsp_connHandle %d\n", __func__, len, offset,ppsp_connHandle);
    ...
    switch ( uuid ) {
      ...
      case PPSP_SERV_CFGS_CHAR_FED5_UUID: {
        if ( PPSP_SERV_CFGS_CHAR_FED5_DLEN <= offset ) {
          ...
          // Write the value
          LOG("Write the value pAttr->pValue %p \n",pAttr->pValue);
...
//        uint16 cfgs = GATTServApp_ReadCharCfg( 0, __ppsp_serv_char_ffd6_cfgs );
		uint16 cfgs = GATTServApp_ReadCharCfg( ppsp_connHandle, __ppsp_serv_char_ffd6_cfgs );
    ...
    // case PPSP_SERV_CFGS_CHAR_5_INDX:
      ...
      case PPSP_SERV_CFGS_CHAR_FFD8_INDX: {
        uint16 cfgs = GATTServApp_ReadCharCfg( ppsp_connHandle, __ppsp_serv_char_ffd8_cfgs );
