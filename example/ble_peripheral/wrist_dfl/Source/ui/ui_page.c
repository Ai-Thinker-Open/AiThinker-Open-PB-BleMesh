...
static int ui_fsm_run(ui_ev_t* pev);
...
static int uipg_new_theme(const ui_ev_t* pev)
{
  ui_ev_t ev_construct = {
          .ev = UI_EV_PAGE_CONSTRUCT,
          .param = 0,
  ...
  s_pagecfg.state = UI_ST_POP;
  s_pagecfg.index = POP_INDEX_NEW_THEME;
  s_pagecfg.data = 0;
...
const ui_page_t s_page_blank[] = {
  ...
  {UI_EV_NEW_THEME,  uipg_new_theme},
  {0,0}
};
...
static int uipg_new_theme_construct(const ui_ev_t* pev)
{
  LOG("uipg_alarm_clk_construct %d\n", s_pagecfg.subst);
  ui_normal_new_theme(UI_STYLE);
  s_pagecfg.subst = 0xff;
  ui_timer_start(3000);
  return APP_SUCCESS;
}

static int uipg_new_theme_key_press(const ui_ev_t* pev)
{
  //vibra_off();
  ui_page_next();
  return 0;
...
static int uipg_new_theme_timer(const ui_ev_t* pev)
{
...
const ui_page_t s_page_pop_new_theme[] = {
  {UI_EV_PAGE_CONSTRUCT,  uipg_new_theme_construct},
  {UI_EV_PAGE_DESTRUCT, uipg_common_destruct},
  {UI_EV_KEY_PRESS,   uipg_new_theme_key_press},
  {UI_EV_TIMER,     uipg_new_theme_timer},
  {0,0}
};
...
static int ui_fsm_dispatch_event(const ui_page_t* ppage, const ui_ev_t* pev){
  int i =0;
  if(ppage == NULL)
...
static void ui_page_timer_evt(void* pctx)
{
  ui_ev_t ev = {
          .ev = UI_EV_TIMER,
          .param = 0,
...
static void ui_accelerator_event(void* pacc)
{
  ui_ev_t ev = {
...
static void ui_batt_event(batt_evt_t evt)
{
  ui_ev_t uiev;
  switch(evt){
...
static void ui_key_evt(void* pctx)
{
  ui_ev_t ev = {
      .ev = UI_EV_KEY_PRESS,
      .param = 0,
...
static int ui_page_config(void)
{
...
const uint32_t dll_table_info[4] __attribute__((section(".ARM.__at_0x20008000"))) =
{
  DFL_FIELD_0,
  DFL_FIELD_1,
  0,
...
const uint32_t* const dll_table_entry[32] __attribute__((section(".ARM.__at_0x20008010"))) =
{
  (const uint32_t*)ui_page_init,
  (const uint32_t*)ui_fsm_run,
  (const uint32_t*)ui_page_timer_evt,
  (const uint32_t*)ui_batt_event,
  (const uint32_t*)ui_key_evt,
  (const uint32_t*)ui_accelerator_event
};
