...
static int cmd_new_theme(const uint8_t* data, uint16_t len)
{
  int ret;
  ui_ev_t ev;
  wristCmdTheme_t* uiindex = (wristCmdTheme_t*)data;

  dfl_load(uiindex->index);

  ev.ev = UI_EV_PAGE_DESTRUCT;
  ret = ui_fsm_run(&ev);
  ret = (ret == PPlus_SUCCESS) ? APP_SUCCESS: APP_ERR_PARAM;
  
  ev.param = uiindex->index;
  ev.ev = UI_EV_NEW_THEME;
  ret = ui_fsm_run(&ev);
  ret = (ret == PPlus_SUCCESS) ? APP_SUCCESS: APP_ERR_PARAM;
  return cmd_response_err(data, len, ret);
}
...
int on_recieved_cmd_packet(const uint8* data, uint16 len)
{
  ...
  case  WRIST_CMD_NEW_THEME:
    ret = cmd_new_theme(data, len);
    break;
