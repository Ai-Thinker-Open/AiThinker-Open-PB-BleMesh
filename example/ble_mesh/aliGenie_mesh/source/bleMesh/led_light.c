...
#define GPIO_WARM   	P14
#define GPIO_COLD    	P15


uint16_t s_light[5];//change by johhn
...
{
  if(s_light[LIGHT_RED] + s_light[LIGHT_GREEN] + s_light[LIGHT_BLUE] + s_light[LIGHT_WARM] + s_light[LIGHT_COLD]){
    ...
    hal_pwm_destroy(PWM_CH2);
		...
		{
			hal_pwm_close_channel(PWM_CH3);
			hal_pwm_destroy(PWM_CH3);
			hal_pwm_close_channel(PWM_CH4);
			hal_pwm_destroy(PWM_CH4);
		}
    hal_pwm_stop();
    ...
    hal_gpio_pull_set(GPIO_BLUE, WEAK_PULL_UP);
		...
		{
		hal_gpio_pin_init(GPIO_WARM, IE);
    hal_gpio_pin_init(GPIO_COLD, IE);
			
    hal_gpio_pull_set(GPIO_WARM, WEAK_PULL_UP);
    hal_gpio_pull_set(GPIO_COLD, WEAK_PULL_UP);
 
		}
    
    if(s_light[LIGHT_GREEN]){
      hal_pwm_init(PWM_CH0, PWM_CLK_DIV_64, PWM_CNT_UP, PWM_POLARITY_RISING);
      hal_pwm_set_count_val(PWM_CH0, LIGHT_TOP_VALUE-s_light[LIGHT_GREEN], LIGHT_TOP_VALUE);//hal_pwm_set_count_val(PWM_CH0, s_light[LIGHT_GREEN], 500);
    ...
    if(s_light[LIGHT_BLUE]){
      ...
      hal_pwm_set_count_val(PWM_CH1, LIGHT_TOP_VALUE-s_light[LIGHT_BLUE], LIGHT_TOP_VALUE);
    ...
    if(s_light[LIGHT_RED]){
      ...
      hal_pwm_set_count_val(PWM_CH2, LIGHT_TOP_VALUE-s_light[LIGHT_RED], LIGHT_TOP_VALUE);
      hal_pwm_open_channel(PWM_CH2, GPIO_RED);
    }
		
		if(s_light[LIGHT_WARM]){
      hal_pwm_init(PWM_CH3, PWM_CLK_DIV_64, PWM_CNT_UP, PWM_POLARITY_RISING);
      hal_pwm_set_count_val(PWM_CH3, LIGHT_TOP_VALUE-s_light[LIGHT_WARM], LIGHT_TOP_VALUE);
      hal_pwm_open_channel(PWM_CH3, GPIO_WARM);
    }
		
		if(s_light[LIGHT_COLD]){
      hal_pwm_init(PWM_CH4, PWM_CLK_DIV_64, PWM_CNT_UP, PWM_POLARITY_RISING);
      hal_pwm_set_count_val(PWM_CH4, LIGHT_TOP_VALUE-s_light[LIGHT_COLD], LIGHT_TOP_VALUE);
      hal_pwm_open_channel(PWM_CH4, GPIO_COLD);
    }
  ...
  else
  {
    ...
    hal_gpio_pull_set(GPIO_BLUE, STRONG_PULL_UP);
		
		hal_gpio_pull_set(GPIO_WARM, STRONG_PULL_UP);
		hal_gpio_pull_set(GPIO_COLD, STRONG_PULL_UP);
...
void light_reflash_johhn_on(void)
{
	 hal_pwm_close_channel(PWM_CH0);
    hal_pwm_destroy(PWM_CH0);
    hal_pwm_close_channel(PWM_CH1);
...
void light_reflash_johhn_off(void)
{
	 hal_pwm_close_channel(PWM_CH0);
    hal_pwm_destroy(PWM_CH0);
    hal_pwm_close_channel(PWM_CH1);
    ...
    hal_gpio_pin_init(GPIO_RED, IE);
    hal_gpio_pin_init(GPIO_BLUE, IE);
    hal_gpio_pull_set(GPIO_GREEN, PULL_DOWN);
    hal_gpio_pull_set(GPIO_RED, PULL_DOWN);
    hal_gpio_pull_set(GPIO_BLUE, PULL_DOWN);
    light_stop_timer();
