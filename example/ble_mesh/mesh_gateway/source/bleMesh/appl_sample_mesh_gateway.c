...
//        ("Send Vendor Model Set\n");
        ...
        if(retval == API_SUCCESS)
        {
            ...
            retval = MS_phymodel_client_send_reliable_pdu
                     (
...
//    UINT16      key_handle;
    ...
    MS_PACK_LE_2_BYTE_VAL(&buffer[marker], MS_STATE_PHY_MODEL_RESET_T);
    marker += 2;
...
//            "MS_ACCESS_PHY_MODEL_WRITECMD_OPCODE\n");
            ...
            {                
                case MS_STATE_PHY_MODEL_HB_CALLBACK_T:
                {
#if (CFG_HEARTBEAT_MODE)                    
    ...
    if (0x01 == req_type->to_be_acked)
    {
        ...
        retval = MS_phy_model_client_state_update(ctx, state_params, NULL, 0, NULL,marker);
    }
            ...
            )
{
    ...
    retval = MS_phy_model_client_init
            (
                element_handle,
                &UI_vendor_defined_client_model_handle,
